...
//    val alarmList = MutableSharedFlow<DataStatus<List<Address>?>>(2)
    val alarmList = MutableStateFlow<DataStatus<List<Address>?>>(DataStatus.Loading)

    init {
        if (BuildConfig.DEBUG && manager.getList().isEmpty()) {
            mutableListOf(
                Address(
                    "스타벅스 석촌호수점",
                    true,
    ...
    fun fetchAlarmList() {
        alarmList.value = DataStatus.Loading
        alarmList.value = DataStatus.Success(manager.getList())
//        flowOf(manager.getList())
//            .onStart { alarmList.emit(DataStatus.Loading) }
//            .catch { e -> alarmList.emit(DataStatus.Failure(e)) }
//            .onEach { alarmList.emit(DataStatus.Success(it)) }
