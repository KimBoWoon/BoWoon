...
class AlarmVM @Inject constructor(
    ...
    fun getGeocode(address: String) {
        viewModelScope.launch {
            coroutineIOCallbackTo(
                ...
                success = { geocode.value = DataStatus.Success(it) },
    ...
    private fun dataMapper(addresses: Addresses): com.data.gpsAlarm.dto.Addresses =
        com.data.gpsAlarm.dto.Addresses(addresses.addressElements?.map { AddressElement(it.code, it.longName, it.shortName, it.types) }, addresses.distance, addresses.englishAddress, addresses.jibunAddress, addresses.roadAddress, addresses.x, addresses.y)
}