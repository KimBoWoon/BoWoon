...
class RssProducer @Inject constructor(
    ...
    companion object {
        private const val TAG = "rss_reader_rss_producer"
    }

    private val handler = CoroutineExceptionHandler { coroutineContext, throwable ->
        Log.d(TAG, "id > [${Thread.currentThread().id}], message > [${throwable.message}]")
    ...
    fun fetch() {
        feeds.forEach { feed ->
            CoroutineScope(Dispatchers.IO + SupervisorJob() + handler).launch {
                Log.d(TAG, "name > [${feed.name}], url > [${feed.url}]")
                when (val result = apis.rssApi.getRss(feed.url ?: "")) {
                    is ApiResponse.Success -> rssChannel.send(result.data)
