...
class PokemonListVM @Inject constructor(
    ...
    val pokemonPageFlow: Flow<PagingData<SealedPokemon>> = Pager(
        PagingConfig(pageSize = 20, initialLoadSize = 20)
    ) {
        PokemonApiSource(pokemonApiUseCase) // TODO 유즈 케이스를 di 사용해야함
    }.flow.map {
        it.insertHeaderItem(item = SealedPokemon.PokemonHeader("PokemonHeader"))
            .insertFooterItem(item = SealedPokemon.PokemonFooter("PokemonFooter"))
