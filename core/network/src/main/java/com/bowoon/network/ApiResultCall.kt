    ...
    override fun enqueue(callback: Callback<ApiResponse<R>>) {
        return delegate.enqueue(object : Callback<R> {
                ...
                when (response.isSuccessful) {
                    true -> {
                        response.body()?.let { body ->
                            callback.onResponse(this@ApiResultCall, Response.success(ApiResponse.Success(body)))
                        } ?: run {
                            if (successType == Unit::class.java) {
                                @Suppress("UNCHECKED_CAST")
                                callback.onResponse(this@ApiResultCall, Response.success(ApiResponse.Success(Unit as R)))
                            } else {
                                ApiResponse.Failure(
                                    throwable = IllegalStateException("Body가 존재하지 않지만, Unit 이외의 타입으로 정의했습니다. ApiResponse<Unit>로 정의하세요.")
                                )
                            }
                        }
                    }
            ...
            override fun onFailure(call: Call<R?>, throwable: Throwable) {
                callback.onResponse(this@ApiResultCall, Response.success(ApiResponse.Failure(throwable = throwable)))
            }
        })
    }
